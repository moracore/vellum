...
export default function DMEdit({ onClose }: Props) {
  const { character, updateCharacter, setDmMode } = useCharacter()
  const [raw, setRaw] = useState(() => character ? JSON.stringify(character, null, 2) : '{}')
  ...
  return (
    <div className="dm-edit-overlay">
      <div className="dm-edit-header">
        <span className="dm-edit-title">DM Edit — {character?.name}</span>
